...
class EquirectangularCropH264CudaNode : public rclcpp::Node {
public:
    EquirectangularCropH264CudaNode()
        : Node("equirectangular_cuda_node")
    {
        ...
        compressed_sub_ = create_subscription<sensor_msgs::msg::CompressedImage>(
            ...
            std::bind(&EquirectangularCropH264CudaNode::compressed_callback, this, std::placeholders::_1));
        equirect_pub_ = create_publisher<sensor_msgs::msg::Image>(output_topic_, rclcpp::SensorDataQoS());

        RCLCPP_INFO(get_logger(), "Compressed-input cropped equirect CUDA node ready");
        ...
        RCLCPP_INFO(get_logger(), "  OpenCV CUDA remap path: %s", use_cuda_remap_ ? "enabled" : "disabled");
    }

    ~EquirectangularCropH264CudaNode() override
    {
        cleanup_decoder();
    }
...
private:
    ...
    void init_cuda_support()
    {
        use_cuda_remap_ = false;

        if (!gpu_enabled_) {
            RCLCPP_INFO(get_logger(), "GPU parameter disabled, using CPU remap path");
            return;
        }

        const int cuda_devices = cv::cuda::getCudaEnabledDeviceCount();
        if (cuda_devices <= 0) {
            RCLCPP_WARN(get_logger(), "OpenCV CUDA is not available at runtime, falling back to CPU remap");
            return;
        }
        ...
        try {
            ...
            RCLCPP_INFO(
                get_logger(),
                "Using OpenCV CUDA device: %s (compute %d.%d)",
                device_info.name(),
                device_info.majorVersion(),
                device_info.minorVersion());
            use_cuda_remap_ = true;
        } catch (const cv::Exception &e) {
            RCLCPP_WARN(get_logger(), "Failed to initialize OpenCV CUDA device: %s", e.what());
        }
    }
    ...
    void init_mapping(int img_height, int img_width)
    {
        ...
        if (use_cuda_remap_) {
            gpu_front_map_x_.upload(front_map_x_);
            gpu_front_map_y_.upload(front_map_y_);
            gpu_back_map_x_.upload(back_map_x_);
            gpu_back_map_y_.upload(back_map_y_);
        }

        maps_initialized_ = true;
    }

    cv::Mat create_equirectangular_cpu(const cv::Mat& front_img, const cv::Mat& back_img)
    {
        cv::Mat front_result;
        cv::Mat back_result;
    ...
    cv::Mat create_equirectangular_cuda(const cv::Mat& front_img, const cv::Mat& back_img)
    {
        try {
            if (front_img.isContinuous()) {
                gpu_front_src_.upload(front_img);
            } else {
                front_contiguous_ = front_img.clone();
                gpu_front_src_.upload(front_contiguous_);
            }

            if (back_img.isContinuous()) {
                gpu_back_src_.upload(back_img);
            } else {
                back_contiguous_ = back_img.clone();
                gpu_back_src_.upload(back_contiguous_);
            }
            ...
            gpu_front_result_.download(front_result_cpu_);
            gpu_back_result_.download(back_result_cpu_);
        } catch (const cv::Exception &e) {
            RCLCPP_WARN_THROTTLE(
                get_logger(),
                *get_clock(),
                5000,
                "OpenCV CUDA remap failed, using CPU fallback: %s",
                e.what());
            use_cuda_remap_ = false;
            return create_equirectangular_cpu(front_img, back_img);
        }

        cv::Mat equirect = cv::Mat::zeros(crop_out_height_, out_width_, CV_8UC3);
        front_result_cpu_.copyTo(equirect, front_mask_);
        back_result_cpu_.copyTo(equirect, back_mask_);
        return equirect;
    }
    ...
    cv::Mat create_equirectangular(const cv::Mat& dual_fisheye_bgr)
    {
        ...
        cv::Mat equirect = use_cuda_remap_
            ? create_equirectangular_cuda(front_img, back_img)
            : create_equirectangular_cpu(front_img, back_img);
    ...
    void init_decoder()
    {
        ...
        if (hw_type_ != AV_HWDEVICE_TYPE_NONE) {
            codec_ = const_cast<AVCodec*>(avcodec_find_decoder_by_name(decoder_name));
        }
        if (!codec_) {
    ...
    void maybe_log_performance()
    {
        ...
        RCLCPP_INFO(
            get_logger(),
            "Compressed equirect CUDA stats: decoded=%.1f fps, published=%.1f fps, avg_decode=%.2f ms, avg_map_init=%.2f ms, avg_remap=%.2f ms, avg_publish=%.2f ms, avg_total=%.2f ms",
            decoded_fps,
            published_fps,
            avg_decode_ms,
            avg_map_init_ms,
    ...
    bool use_cuda_remap_ = false;
    cv::cuda::GpuMat gpu_front_map_x_;
    cv::cuda::GpuMat gpu_front_map_y_;
    cv::cuda::GpuMat gpu_back_map_x_;
    cv::cuda::GpuMat gpu_back_map_y_;
    cv::cuda::GpuMat gpu_front_src_;
    cv::cuda::GpuMat gpu_back_src_;
    cv::cuda::GpuMat gpu_front_result_;
    cv::cuda::GpuMat gpu_back_result_;
    cv::Mat front_result_cpu_;
    cv::Mat back_result_cpu_;
    cv::Mat front_contiguous_;
    cv::Mat back_contiguous_;
...
int main(int argc, char** argv)
{
    ...
    auto node = std::make_shared<EquirectangularCropH264CudaNode>();
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}
