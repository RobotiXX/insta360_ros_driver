...
private:
    ...
    uint64_t dropped_due_backlog_ = 0;
    uint64_t dropped_due_pacing_ = 0;
    std::chrono::steady_clock::time_point last_enqueued_frame_time_{};
    std::chrono::steady_clock::time_point backlog_log_window_start_ = std::chrono::steady_clock::now();
...
public:
    GstDecodedNode()
        : Node("insta360_decoded_gst_hwonly_node")
    {
    ...
    bool startCamera()
    {
        ...
        RCLCPP_INFO(get_logger(), "Started strict hardware-only GStreamer decode node.");
        return true;
    }
...
private:
    ...
    bool startDecoderPipeline()
    {
        ...
        if (!pipeline_ || !appsrc_ || !parse || !queue || !decoder || !nvvidconv || !videoconvert || !capsfilter || !appsink_) {
        ...
        g_object_set(G_OBJECT(appsrc_),
            ...
            "do-timestamp", TRUE,
            "block", FALSE,
            "max-bytes", static_cast<guint64>(256 * 1024),
            "caps", src_caps,
            nullptr);
        gst_caps_unref(src_caps);

        GParamSpec* appsrc_max_buffers_spec = g_object_class_find_property(G_OBJECT_GET_CLASS(appsrc_), "max-buffers");
        if (appsrc_max_buffers_spec) {
            g_object_set(G_OBJECT(appsrc_), "max-buffers", 4u, nullptr);
        }

        GParamSpec* appsrc_max_time_spec = g_object_class_find_property(G_OBJECT_GET_CLASS(appsrc_), "max-time");
        if (appsrc_max_time_spec) {
            g_object_set(G_OBJECT(appsrc_), "max-time", static_cast<guint64>(200000000), nullptr);
        }
        ...
        const bool linked = gst_element_link_many(appsrc_, parse, queue, decoder, nvvidconv, videoconvert, capsfilter, appsink_, nullptr);
    ...
    int stream_index)
{
    ...
    const bool is_idr = isIdrFrame(data, size);

    if (owner_->low_latency_mode_ && !is_idr) {
        const auto now_tp = std::chrono::steady_clock::now();
        if (last_enqueued_frame_time_.time_since_epoch().count() != 0) {
            const auto since_last_us = std::chrono::duration_cast<std::chrono::microseconds>(
                now_tp - last_enqueued_frame_time_).count();
            if (since_last_us < 70000) {
                ++dropped_due_pacing_;
                const auto elapsed_ms = std::chrono::duration_cast<std::chrono::milliseconds>(
                    now_tp - backlog_log_window_start_).count();
                if (elapsed_ms >= 2000) {
                    RCLCPP_WARN(
                        owner_->get_logger(),
                        "Dropping compressed frames for low latency (pacing drops=%lu, backlog drops=%lu)",
                        static_cast<unsigned long>(dropped_due_pacing_),
                        static_cast<unsigned long>(dropped_due_backlog_));
                    dropped_due_pacing_ = 0;
                    dropped_due_backlog_ = 0;
                    backlog_log_window_start_ = now_tp;
                }
                return;
            }
        }
    ...
    if (owner_->low_latency_mode_ && !is_idr) {
        const guint64 level_bytes = gst_app_src_get_current_level_bytes(appsrc_);
        if (level_bytes > static_cast<guint64>(128 * 1024)) {
            ++dropped_due_backlog_;
            const auto now_tp = std::chrono::steady_clock::now();
            const auto elapsed_ms = std::chrono::duration_cast<std::chrono::milliseconds>(
                now_tp - backlog_log_window_start_).count();
            if (elapsed_ms >= 2000) {
                RCLCPP_WARN(
                    owner_->get_logger(),
                    "Dropping compressed frames to reduce latency (appsrc backlog=%lu bytes, dropped=%lu)",
                    static_cast<unsigned long>(level_bytes),
                    static_cast<unsigned long>(dropped_due_backlog_));
                dropped_due_backlog_ = 0;
                backlog_log_window_start_ = now_tp;
            }
    ...
    if (ret != GST_FLOW_OK && ret != GST_FLOW_FLUSHING) {
        ...
        last_enqueued_frame_time_ = std::chrono::steady_clock::now();
    }
}
...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        auto logger = rclcpp::get_logger("insta360_decoded_gst_hwonly_node");
